...
internal sealed class InstallCommand : ISLCommand
{
    ...
    public string HelpContent =>
        ...
        $"  |子命令1: -java[=<type>+<version>+<platform>(+kind)] | -j | -Java: 下载指定的java (筛选器语法在'-j' 与 '-Java'子命令中也相同){Environment.NewLine}" +
        $"  |用法: 需求举例: 下载jdk17, 目标平台为linux: {Environment.NewLine}" +
        $"  |  | install -java=jdk+17+linux{Environment.NewLine}" +
        ...
        $"  |{Environment.NewLine}" +
        $"  |子命令2: -core[=<coreID>+<modifier>*] | -c | -Core: 下载根据修饰器修饰的版本，并按照修饰器对其进行配置 (修饰器语法在'-c' 与 '-Core'子命令中也相同){Environment.NewLine}" +
        $"  |用法: 需求举例1: 下载一个1.18.2的核心并使用Fabric安装器: {Environment.NewLine}" +
        $"  |  | install -core=1.18.2+fabric{Environment.NewLine}" +
